...
<div class="mc-table-card mt-4 p-4" id="photoGalleryCard">
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
            <h6 class="fw-bold mb-0"><i class="bi bi-images me-2 text-success"></i>Fotos del negocio</h6>
            <p class="text-muted small mb-0">Sube hasta 30 fotos. Máx. 5 MB por imagen. (JPEG, PNG o WebP)</p>
        </div>
        <label class="btn btn-outline-success btn-sm" id="photoUploadLabel" style="cursor:pointer">
    ...
    <p id="photoEmpty" class="text-muted small text-center py-3 d-none">No hay fotos aún. ¡Sube la primera!</p>
</div>
...
(function(){
    ...
    function renderGrid(photos){
        ...
        photos.forEach(function(p){
            ...
            col.dataset.id = p.id;
            col.innerHTML =
                '<div class="position-relative" style="aspect-ratio:1;overflow:hidden;border-radius:10px;background:#f8f9fa">' +
                    '<img src="' + escHtml(p.url) + '" alt="' + escHtml(p.caption || '') + '" ' +
                         'style="width:100%;height:100%;object-fit:cover">' +
                    '<button class="photo-del-btn btn btn-sm position-absolute top-0 end-0 m-1 p-1 lh-1" ' +
                            'style="background:rgba(220,53,69,0.85);border:none;color:#fff;border-radius:6px" ' +
                            'title="Eliminar" data-id="' + p.id + '">' +
    ...
    grid.addEventListener('click', function(e){
        ...
        fetch(BASE + '/dashboard/settings/photos/' + id + '/delete', {
          ...
          .then(function(d){ if (d.ok) btn.closest('[data-id]').remove(); if (!grid.children.length) emptyMsg.classList.remove('d-none'); })
          .catch(function(){});
    });
